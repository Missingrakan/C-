...
///////////////////////////////////////////////////////////////////////
AutoGetResultTable::AutoGetResultTable(SqliteManager *db, const string &sql, int &row, int &col, char **&ppRet) : m_db(db)
{
...
void DataManager::InsertDoc(const string &path, const string &doc)
{
	...
	sprintf(sql, "insert into %s values(null, '%s', '%s', '%s', '%s')", DOC_TABLE, doc.c_str(), path.c_str(), pinyin.c_str(), initials.c_str());
	m_dbmgr.ExecuteSql(sql);
}
	...
	if (pos != string::npos)
	{
		...
		while (str_index < strlower.size())
		{
			...
			else
			{
				...
				string word_pinyin = ChineseConvertPinYinAllSpell(word);
